...
export default function TranslationCard({
  ...
  trProvider, setTrProvider, trSrc, setTrSrc,
  trPair, setTrPair, orModel, lastSrtMap, onTranslate
}) {
  const logRef  = useRef(null)
  const fileRef = useRef(null)
  useEffect(() => { if (logRef.current) logRef.current.scrollTop = logRef.current.scrollHeight }, [trLog])
    ...
    <div className="card card-tr">
      ...
      <div style={{ marginTop:12 }}>
        ...
        <div style={{ display:'flex', gap:6 }}>
          <button className={`btn tm tm-smart${trProvider==='gm'?' on':''}`} onClick={() => setTrProvider('gm')}>Gemini</button>
          <button className={`btn tm tm-vosk${trProvider==='or'?' on':''}`} onClick={() => setTrProvider('or')}>
      ...
      <div className="ar" style={{ marginTop:14 }}>
        <button className="btn bpp" disabled={trRunning} onClick={() => onTranslate({ trFileRef: fileRef })}>
          🌐 Перевести
        </button>
      ...
      <div className="logbox logbox-sm" ref={logRef} style={{ marginTop:12 }}>
        {trLog.length === 0
          ...
          : trLog.map(entry => <div key={entry.id} className={entry.cls}>{entry.msg}</div>)
        }
      </div>
    </div>
  )
