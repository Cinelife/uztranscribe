...
export async function orGenerateChunk(key, model, b64, dur, langName, maxChars, onLog) {
  ...
  if (!raw.trim()) { onLog(`    ⚠ OR: пустой ответ от ${model}`, 'wa'); return [] }
...
export async function transcribeOpenRouter(file, key, model, lang, chunkSec, maxChars, preChunks, onLog, onProgress, stopFlagRef) {
  ...
  for (let ci = 0; ci < chunks.length; ci++) {
    ...
    for (let att = 1; att <= 2; att++) {
      try {
        const r = await orGenerateChunk(key, model, b64, dur, langName, maxChars, onLog)
    ...
    if (segs.length === 0) onLog(`    ⚠ чанк ${ci+1} → 0 сегментов (OR вернул пусто)`, 'wa')
    ...
    if ((ci + 1) % 10 === 0 && ci < chunks.length - 1) { onLog(`    ⏸ пауза 1с...`, 'wa'); await sleep(1000) }
...
export async function translateBatchOR(segs, pair, key, model) {
  ...
  const tryP = txt => {
    ...
    return segs.map((seg, idx) => { const f = p.find(x => x.i === idx); return { ...seg, text: f ? f.t : seg.text } })
  }
  try { return tryP(raw) }
  catch (_) {
