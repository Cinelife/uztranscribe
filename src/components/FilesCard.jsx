...
export default function FilesCard({ files, setFiles, fileStatuses }) {
  ...
  const addFiles = (list) => {
    setFiles(prev => {
      ...
      Array.from(list).forEach(f => {
        ...
        if (ALLOWED.has(ext) && !next.find(x => x.name === f.name && x.size === f.size)) next.push(f)
      })
      return next
    })
  }
  ...
  return (
    <div className="card">
      ...
      >
        <input type="file" multiple accept=".mp3,.wav,.m4a,.ogg,.flac,.aac,.mp4,.mov,.mkv,.avi,.webm"
      ...
      {files.length > 0 && (
        <div style={{ marginTop:8 }}>
          <button className="btn bx" onClick={clearFiles} style={{ fontSize:10, padding:'4px 10px' }}>Очистить все</button>
        </div>
      )}
    </div>
  )
