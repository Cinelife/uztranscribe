...
export function useTranslation() {
  ...
  const addTrLog = useCallback((msg, cls = '') => { const id = logIdRef.current++; setTrLog(prev => [...prev, { id, msg, cls }]) }, [])
  const clearTrLog = useCallback(() => { setTrLog([{ id: logIdRef.current++, msg: '// Лог очищен', cls: 'dm' }]) }, [])
  const startTranslate = useCallback(async ({ gmKey, orKey, orModel, trProvider, trSrc, trPair, lastSrtMap, trFileRef }) => {
    ...
    for (let bi = 0; bi < batches.length; bi++) {
      addTrLog(`   Батч ${bi+1}/${batches.length}...`, 'dm'); setTrStatus(`${bi+1}/${batches.length}`)
      try {
        const res = trProvider === 'or' ? await translateBatchOR(batches[bi], trPair, orKey, orModel) : await translateBatch(batches[bi], trPair, gmKey)
        translated.push(...res); addTrLog(`   ✓ Батч ${bi+1}: ${res.length} сегм.`, 'ok')
      } catch (e) { addTrLog(`   ✗ Батч ${bi+1}: ${e.message}`, 'er'); translated.push(...batches[bi]); ok = false }
