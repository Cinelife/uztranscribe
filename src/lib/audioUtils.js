...
/**
 * v11: Extract a slice of AudioBuffer without re-decoding the file.
 * Returns a new 16kHz mono AudioBuffer for Vosk input.
 */
export function sliceToAudioBuffer(ab, t0, t1) {
  ...
  const sliced = offCtx.createBuffer(1, len, sr)
  const nc = ab.numberOfChannels
...
/**
 * v11: Group Vosk word array into segments by pause gaps.
 * Returns [{start, end}] — acoustic boundaries only, no text.
 * @param {Array<{word, start, end}>} words  — 0-relative to chunk
 * @param {number} minPause  — min gap in seconds to split (default 0.3)
 * @param {number} maxSegDur — max segment duration in seconds (default 7.0)
 */
