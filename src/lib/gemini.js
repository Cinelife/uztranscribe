...
export async function transcribeGemini(file, key, lang, chunkSec, maxChars, preChunks, onLog, onProgress, stopFlagRef) {
  ...
  for (let ci = 0; ci < chunks.length; ci++) {
    ...
    for (let att = 1; att <= 2; att++) {
      try { const r = await geminiGenerateChunk(key, b64, dur, langName, maxChars, onLog); segs = Array.isArray(r) ? r : (r && typeof r === "object" && "text" in r ? [r] : []); break }
      catch (e) {
    ...
    if (segs.length === 0) onLog(`    ⚠ чанк ${ci+1} → 0 сегментов (Gemini вернул пусто)`, 'wa')
