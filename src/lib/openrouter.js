...
export async function orGenerateChunk(key, model, b64, dur, langName, maxChars, onLog) {
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json', 'HTTP-Referer': 'https://cinelife.github.io/uztranscribe', 'X-Title': 'Uzbek Transcriber' },
    body: JSON.stringify({ model, messages: [{ role: 'user', content: [{ type: 'input_audio', input_audio: { data: b64, format: 'wav' } }, { type: 'text', text: buildPrompt(dur, langName, maxChars) }] }], temperature: 0, max_tokens: 4096 })
  })
  ...
  if (!r.ok) { const txt = (await r.text()).slice(0, 200); throw new Error(`OpenRouter ${r.status}: ${txt}`) }
  const d = await r.json()
  if (d.error) throw new Error(`OpenRouter error: ${d.error.message || JSON.stringify(d.error)}`)
  const raw = d.choices?.[0]?.message?.content || ''
  if (!raw.trim()) { onLog(`    ⚠ OR: пустой ответ`, 'wa'); return [] }
...
export async function transcribeOpenRouter(file, key, model, lang, chunkSec, maxChars, preChunks, onLog, onProgress, stopFlagRef) {
  ...
  let chunks = preChunks || (onLog(`    OR: поиск тихих мест...`, 'or-cl'), buildSmartChunks(ab, chunkSec))
  onLog(`    OR: ${ab.duration.toFixed(1)}с → ${chunks.length} чанков [${preChunks ? 'Vosk-границы' : 'Smart Silence'}]`, preChunks ? 'ok' : 'or-cl')
  const allSegs = []
  for (let ci = 0; ci < chunks.length; ci++) {
    if (stopFlagRef.current) break
    const { t0, t1 } = chunks[ci], dur = t1 - t0
    onLog(`    OR: чанк ${ci+1}/${chunks.length} [${t0.toFixed(1)}–${t1.toFixed(1)}s]`, 'dm')
    onProgress(`⏳ OR чанк ${ci+1}/${chunks.length}`)
    const b64 = await blobToBase64(sliceToWav(ab, t0, t1))
    let segs = []
    for (let att = 1; att <= 2; att++) {
      try { segs = await orGenerateChunk(key, model, b64, dur, langName, maxChars, onLog); break }
      catch (e) {
    ...
    if ((ci + 1) % 10 === 0 && ci < chunks.length - 1) { await sleep(1000) }
    else if (ci < chunks.length - 1) await sleep(700)
  }
...
export async function translateBatchOR(segs, pair, key, model) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json', 'HTTP-Referer': 'https://cinelife.github.io/uztranscribe', 'X-Title': 'Uzbek Transcriber' },
    body: JSON.stringify({ model, messages: [{ role: 'user', content: prompt }], temperature: 0.2, max_tokens: 8192 })
  })
  if (r.status === 429) throw new Error('OpenRouter 429: Rate limit.')
  ...
  raw = raw.trim()
  const tryP = txt => { const p = JSON.parse(txt); return segs.map((seg, idx) => { const f = p.find(x => x.i === idx); return { ...seg, text: f ? f.t : seg.text } }) }
  try { return tryP(raw) } catch (_) {
