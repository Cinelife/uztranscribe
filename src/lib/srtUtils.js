export function srtTime(s) {
  const ms = Math.round((s % 1) * 1000); s = Math.floor(s)
  const h = Math.floor(s / 3600), m = Math.floor((s % 3600) / 60), sec = s % 60
  return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(sec).padStart(2,'0')},${String(ms).padStart(3,'0')}`
}
export function buildSrt(segs) {
  return segs.map(s => { if (s.end < s.start) { const t = s.start; s.start = s.end; s.end = t } return s })
...
export function splitLongLines(segs, maxChars) {
  ...
  for (const s of segs) {
    ...
    const words = s.text.split(' '), mid = Math.ceil(words.length / 2), dur = (s.end - s.start) / 2
