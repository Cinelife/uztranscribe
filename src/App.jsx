...
export default function App() {
  ...
  const {
    log, clearLog, progress, progressText, statusText,
    voskVisible, voskPct, voskText, running, startBatch, stopBatch, lastSrtMap
  } = useBatchRunner()

  const { trLog, clearTrLog, trStatus, trRunning, startTranslate } = useTranslation()

  const handleStart = () => startBatch({
    files, prov, lang, chunkSec, maxChars, timingMode,
    elKey, gmKey, orKey, orModel, voskReady, voskModelRef
  })

  const handleTranslate = ({ trFileRef }) => startTranslate({
    gmKey, orKey, orModel, trProvider, trSrc, trPair, lastSrtMap, trFileRef
  })
  ...
  return (
    <div className="wrap">
      ...
      <div className="info">
        ...
        🔬 <strong>Vosk v11</strong> &nbsp;|&nbsp;
        🌐 Перевод с культурной адаптацией
      </div>
      <ApiKeysCard elKey={elKey} gmKey={gmKey} orKey={orKey}
        setElKey={setElKey} setGmKey={setGmKey} setOrKey={setOrKey} />
      <SettingsCard
        prov={prov} setProv={setProv} lang={lang} setLang={setLang}
      ...
      <ProgressCard
        ...
        progress={progress} progressText={progressText} statusText={statusText}
        voskVisible={voskVisible} voskPct={voskPct} voskText={voskText}
        running={running} onStart={handleStart} onStop={stopBatch}
      />
      <TranslationCard
        ...
        orModel={orModel} lastSrtMap={lastSrtMap}
