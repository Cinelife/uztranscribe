...
export default function SettingsCard({
  ...
  return (
    <div className="card">
      ...
      {(prov === 'or' || prov === 'bo') && (
        <div className="or-row">
          <label style={{ marginTop:0 }}>
            Модель OpenRouter{' '}
            <span style={{ color:'var(--mu)' }}>(для транскрипции и перевода)</span>
          </label>
      ...
      {/* Timing mode (Gemini / OR only) */}
      <div className="vosk-row">
        <label style={{ marginTop:0 }}>
          Метод тайм-кодов{' '}
          <span style={{ color:'var(--mu)', fontSize:10 }}>(только для Gemini / OpenRouter)</span>
        </label>
        ...
        {timingMode === 'vosk' && (
          <div style={{ marginTop:12 }}>
            <label style={{ marginTop:0 }}>
              Модель Vosk{' '}
              <span style={{ color:'var(--mu)' }}>(vosk-model-small-uz-0.22.zip — 49 МБ, загружается один раз)</span>
            </label>
            <div className="kr">
              <input type="file" ref={voskFileRef} accept=".zip"
                onChange={() => !voskReady && handleLoadVosk()} />
              <button className="btn bc" ref={voskBtnRef} onClick={handleLoadVosk}>
                ▶ Загрузить
              </button>
            </div>
